...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    const existing = await db.findUserByEmail(email); // <-- ADD await!
    ...
    const userId = await db.createUser(name, email, passwordHash); // <-- ADD await!

    req.session.userId = userId;

    const safeUser = await db.findUserById(userId); // <-- ADD await!
...
app.post('/api/auth/logout', (req, res) => {
    ...
    req.session.destroy(err => {
        if(err){
            ...
            return rmSync.json(500).json({error : 'Nem sikerült kijelentkezni.'})
        }
